...
class Test final : public CApplication
{
    ...
    static constexpr unsigned StaticCmdSize = 128*1024;
...
public:
    Test()
    {
        ...
        pool_data.emplace(device, DkMemBlockFlags_CpuUncached | DkMemBlockFlags_GpuCached, 16*1024*1024);
        pool_code.emplace(device, DkMemBlockFlags_CpuUncached | DkMemBlockFlags_GpuCached | DkMemBlockFlags_Code, 2*1024*1024);
        ...
        for (unsigned i = 0; i < NumFramebuffers; i ++)
        {
            ...
            for (int k = 0; k < 10; ++k) {
                for (int j = 0; j < 10; ++j) {
                    ...
                    view.setLayers(j, 1);
                    view.setMipLevels(k, 1);
                    cmdbuf.bindRenderTargets(&view);
                    cmdbuf.clearColor(0, DkColorMask_RGBA, 0.2f+j*0.08f, 0.0f, k ? 1.0f : 0.0f, 1.0f);
                }
            }

...
public:
    ...
    void createFramebufferResources()
    {
        ...
        auto input_allocation = pool_images->allocate(DIM * DIM * DIM * 4, 4);
        auto input_block = input_allocation.getMemBlock();
        u8* ptr = (u8*)input_block.getCpuAddr();
        memset(ptr, 0xff, input_block.getSize());

        for (u32 z = 0; z < DIM; ++z)
        for (u32 y = 0; y < DIM; ++y)
        for (u32 x = 0; x < DIM; ++x) {
            const u32 d = z * DIM * DIM + y * DIM + x;
            ptr[d * 4] = 0xff;
        }
        ...
        memset(test_block.getCpuAddr(), 16, test_block.getSize());
        ...
        for (size_t i = 0; i < descriptors.size(); ++i) {
            descriptors[i].initialize(dk::ImageView{test_image}.setMipLevels(0, 1));
        }
        ...
        sampler.setBorderColor(0.0f, 0.5f, 0.0f, 1.0f);
        ...
        for (unsigned i = 0; i < NumFramebuffers; i ++)
        {
            ...
            for (uint32_t j = 0; j < DIM; ++j) {
                ...
                DkCopyBuf ref{
                    .addr = input_block.getGpuAddr(),
                };
                DkImageRect r{
                    .x = 0, .y = 0, .z = 0,
                    .width = DIM, .height = DIM, .depth = 1,
                };
                cmdbuf.copyBufferToImage(ref, view, r, 0);
            }

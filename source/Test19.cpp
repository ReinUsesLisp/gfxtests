...
public:
    ...
    void createFramebufferResources()
    {
        ...
        gradientShader.load(*pool_code, "romfs:/shaders/gradient_fsh.dksh");
        ...
        for (unsigned i = 0; i < NumFramebuffers; i ++)
        {
            ...
            cmdbuf.setViewports(0, {{ 0, 0, DIM, DIM }});

            cmdbuf.bindShaders(DkStageFlag_GraphicsMask, { vertexShader, gradientShader });

            std::array<DkImageView, 8> data_views;
            std::array<DkImageView*, 8> views;
            for (uint32_t j = 0; j < DIM; ++j) {
                data_views[j] = dk::ImageView{test_image}.setLayers(j, 1);
                views[j] = &data_views[j];
            }
            cmdbuf.bindRenderTargets( dk::detail::ArrayProxy<const DkImageView* const>(8, views.data()) );

...
public:
    ...
    void createFramebufferResources()
    {
        ...
        std::array<dk::ImageDescriptor, 10> descriptors;
        for (size_t i = 0; i < descriptors.size(); ++i) {
            descriptors[i].initialize(dk::ImageView{test_image}.setMipLevels(i, 1));
        }
        ...
        for (unsigned i = 0; i < NumFramebuffers; i ++)
        {
            ...
            for (int j = 0; j < 10; ++j) {
                ...
                view.setMipLevels(j, 1);
                cmdbuf.bindRenderTargets(&view);
                cmdbuf.clearColor(0, DkColorMask_RGBA, 0.0f, 0.0f, 0.2f+j*0.08f, 1.0f);
            }
            ...
            for (int j = 0; j < 10; ++j) {
                ...
                cmdbuf.bindTextures(DkStage_Fragment, 0, dkMakeTextureHandle(j, 0));
